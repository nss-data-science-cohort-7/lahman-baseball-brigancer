...
FROM fielding SELECT playerid, yearid, pos
...
FROM teams SELECT yearid, hr, so, 
...
FROM people SELECT playerid, namefirst
...
FROM seriespost SELECT yearid, teamidwinner, wins, 
FROM teams SELECT teamid, yearid, w
...
FROM pitching SELECT playerID, yearID, W, GS>10, SO
FROM SALARIES SELECT yearid, playerid, teamid?
...
FROM people SELECT playerid, namefirst, namelast, debut, finalgame
FROM batting SELECT playerid, yearid, hr
